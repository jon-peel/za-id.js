...
export function validateIdNumber(idNumber: string, details? :idNumberDetails): boolean {
	...
	if (idArray[southAfricanCheck] > 1 || idArray[penultimate] < 8 || !validateArray(idArray)) return false;
...
function validateArray(idArray: number[]) {
    ...
    const evens = reduce(map(filter(idArray,evenPredicate), evenMap), reduction, 0);
